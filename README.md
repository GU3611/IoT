...
- 예제 6-1 Person.prototype
  - Person.prototype을 이용해 메서드를 프로토타입에 추가하는 방법.
- 예제 6-2 Prototyper __proto__
  - 인스턴스의 __proto__를 통해 프로토타입 체인 확인.
- 예제 6-3 constructor프로퍼티
  - 인스턴스의 constructor 프로퍼티 확인 방법.
- 예제 6-4 constructor 변경
  - constructor 프로퍼티를 수동으로 변경하는 예.
- 예제 6-5 다양한 constructor 접근 방법
  - 다양한 방식으로 constructor에 접근하는 방법 비교.
- 예제 6-6 메서드 오버라이드
  - 인스턴스에서 메서드를 오버라이드(재정의)하는 방법.
- 예제 6-7 배열에서 배열 메서드 및 객체 메서드 실행
  - 배열에서 배열 메서드와 객체 메서드의 실행 차이.
- 예제 6-8 메서드 오버라이드와 프로토타입 체이닝
  - 오버라이드된 메서드와 프로토타입 체이닝의 동작 방식.
- 예제 6-9 Object.prototype에 추가한 메서드에의 접근
  - Object.prototype에 추가한 메서드의 접근 방식.
- 예제 6-10 Grade 생성자 함수와 인스턴스
  - Grade 생성자 함수 정의와 인스턴스 생성.

### Chapter 7
- 예제 7-1 스태틱 메서드, 프로토타입 메서드
  - 클래스의 정적 메서드와 프로토타입 메서드의 차이.
- 예제 7-2 6-2-4절의 Grade 생성자 함수 및 인스턴스
  - 앞선 Grade 생성자 함수와 인스턴스를 다시 활용.
- 예제 7-3 length 프로퍼티를 삭제한 경우
  - 배열의 length 프로퍼티를 삭제했을 때의 영향.
- 예제 7-4 요소가 있는 배열을 prototype에 매칭한 경우
  - 배열의 프로토타입을 사용자 정의 객체로 바꾼 사례.
- 예제 7-5 Rectangle, Square 클래스
  - Rectangle과 Square 클래스 정의.
- 예제 7-6 Square 클래스 변형
  - Square 클래스를 약간 수정한 변형 예.
- 예제 7-7 Rectangle을 상속하는 Square 클래스
  - Square가 Rectangle을 상속하는 구조 구현.
- 예제 7-8 클래스 상속 및 추상화 방법(1) - 인스턴스 생성 후 프로퍼티 제거
  - 인스턴스 생성 후 프로퍼티를 제거하는 추상화 기법.
- 예제 7-9 클래스 상속 및 추상화 방법(2) - 빈 함수를 활용
  - 추상화를 위해 빈 함수를 상속 구조로 사용하는 방식.
- 예제 7-10 클래스 상속 및 추상화 방법(3) - Object.create 활용
  - Object.create를 활용한 클래스 상속 예시.
- 예제 7-11 클래스 상속 및 추상화 방법 - 완성본(1) - 인스턴스 생성 후 프로퍼티 제거
  - 추상화 완성본 (1) - 인스턴스 생성 후 프로퍼티 제거 방식.
- 예제 7-12 클래스 상속 및 추상화 방법 - 완성본(2) - 빈 함수를 활용
  - 추상화 완성본 (2) - 빈 함수 방식 적용.
- 예제 7-13 클래스 상속 및 추상화 방법 - 완성본(3) - Object.create 활용
  - 추상화 완성본 (3) - Object.create 방식 활용.
- 예제 7-14 상위 클래스 접근 수단인 super 메서드 추가
  - 상위 클래스 접근을 위한 super 메서드 추가 예.
- 예제 7-15 ES5와 ES6의 클래스 문법 비교
  - ES5와 ES6 클래스 문법의 비교.
- 예제 7-16 ES6의 클래스 상속
  - ES6 클래스 문법을 통한 상속 구현 예.
